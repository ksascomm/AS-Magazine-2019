...
if ( ! class_exists( 'asmagazine_Comments' ) ) :
	class asmagazine_Comments extends Walker_Comment {
		...
		function __construct() { ?>
			...
			<h3><?php comments_number( __( 'No Responses to', 'asmagazine' ), __( 'One Response to', 'asmagazine' ), __( '% Responses to', 'asmagazine' ) ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
		...
		function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
			...
			<header class="comment-author">
				...
				printf(
					__( '<cite class="fn">%s</cite>', 'asmagazine' ),
				...
				<section id="comment-content-<?php comment_ID(); ?>" class="comment">
					<?php if ( ! $comment->comment_approved ) : ?>
							<div class="notice">
					<p class="bottom"><?php _e( 'Your comment is awaiting moderation.', 'asmagazine' ); ?></p>
				</div>
						<?php
					else :
						comment_text();
